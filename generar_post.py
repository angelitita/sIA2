# --- EJECUTANDO SCRIPT v15.0: HÍBRIDO (NOTICIAS REALES + CONTENIDO IA) ---
import os
import datetime
import json
...
print("--- INICIANDO SCRIPT DE GENERACIÓN DE CONTENIDO v15.0 ---")
...
# --- LISTAS DE TEMAS PARA CONTENIDO GENERADO ---
temas_opinion = [
    ...
    "un análisis crítico de las gafas Ray-Ban Meta. ¿Son realmente útiles en el día a día o solo un juguete caro?",
    "una opinión sobre Suno AI para la creación de música. ¿Puede realmente reemplazar a los músicos?"
]
temas_herramientas = [
    "una comparativa detallada: Midjourney vs. Stable Diffusion para artistas digitales.",
    "una guía definitiva de las 5 mejores IAs para editar video en 2025.",
    "una reseña a fondo de Notion AI, con pros, contras y un caso de uso práctico."
]

# --- PLANTILLAS HTML (Restauradas) ---
...
def obtener_noticia_real_de_rss():
    ...
    for feed_url in RSS_FEEDS:
        ...
        if feed.entries:
            ...
            if ultima_noticia.link not in historial:
                print(f"✅ Noticia real encontrada: '{ultima_noticia.title}'")
                return {"titulo": ultima_noticia.title, "link": ultima_noticia.link, "resumen": BeautifulSoup(ultima_noticia.summary, "html.parser").get_text(separator=' ', strip=True)}
    return None
...
def reescribir_noticia_con_ia(noticia):
    ...
    system_prompt = "Eres un periodista para 'sIA'. Tu tarea es reescribir noticias de otras fuentes en un artículo original y atractivo. DEBE estar en español."
    user_prompt = f"""Basado en esta información: Título: "{noticia['titulo']}", Resumen: "{noticia['resumen']}", Fuente: "{noticia['link']}", escribe un artículo. Formato JSON: {{"title": "...", "summary": "...", "content_html": "..."}}"""
    try:
        chat_completion = client_groq.chat.completions.create(messages=[{"role": "system", "content": system_prompt}, {"role": "user", "content": user_prompt}], model="llama3-8b-8192", response_format={"type": "json_object"})
        contenido = json.loads(chat_completion.choices[0].message.content)
...
def get_post_details(file_path):
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            ...
            category = category_tag.string.strip() if category_tag else "Noticias" # Fallback
...
def actualizar_paginas(todos_los_posts):
    ...
    for categoria, posts in posts_por_categoria.items():
        ...
        grid_categoria = crear_grid_html(posts, len(posts)) # Muestra todos
...
if __name__ == "__main__":
    ...
    # Decisión: Priorizar noticias reales, si no hay, generar contenido IA
    ...
    if noticia_real:
        ...
        # Decidir entre Opinión o Herramientas si no hay noticias
